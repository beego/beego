...
func (input *BeegoInput) CopyBody(MaxMemory int64) []byte {
	...
	if input.Context.Request.Body == nil {
		readable = reader{bytes.NewBuffer(nil)}
	} else {
		buf, _ := ioutil.ReadAll(input.Context.Request.Body)
		readable = reader{bytes.NewBuffer(buf)}
	}
	safe := &io.LimitedReader{R: readable, N: MaxMemory}
