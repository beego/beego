...
func TestCheck(t *testing.T) {
	if exists := safeMap.Check("cdle"); !exists {
		t.Error("expected", true, "got", false)
	}
}

func TestGet(t *testing.T) {
	if val := safeMap.Get("cdle"); val.(int) != 1 {
...
func TestItems(t *testing.T) {
	safeMap := NewBeeMap()
	safeMap.Set("cdle", "hello")
	for k, v := range safeMap.Items() {
		key := k.(string)
		value := v.(string)
		if key != "cdle" {
			t.Error("expected the key should be cdle")
		}
		if value != "hello" {
