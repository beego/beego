...
func TestWrapperFromJson(t *testing.T) {
	type MyStruct1 struct {
		Foo string `json:"foo"`
	}

	contentType := "application/json"
	testFunc := func(ctx *context.Context, s1 MyStruct1) (any, error) {
		return s1, nil
	}

	app := web.NewHttpSever()
	app.Cfg.CopyRequestBody = true
	app.Post("/api/data", WrapperFromJson(testFunc))
	reqBody := bytes.NewBuffer([]byte(`{"foo": "bar"}`))

	req := httptest.NewRequest("POST", "/api/data", reqBody)
	...
	assert.Equal(t, http.StatusOK, w.Code, "HTTP 状态码错误")
	assert.Equal(t, "{\"foo\":\"bar\"}", w.Body.String(), "响应内容错误")
}

func TestWrapperFromForm(t *testing.T) {
	type MyStruct1 struct {
		Foo string `form:"foo" json:"foo"`
	}
	...
	app := web.NewHttpSever()
	app.Cfg.CopyRequestBody = true
	app.Post("/api/data", WrapperFromForm(testFunc))

	formData := url.Values{}
	formData.Set("foo", "bar")

	req := httptest.NewRequest("POST", "/api/data", strings.NewReader(formData.Encode()))
	req.Header.Set("Content-Type", contentType)
	req.Header.Set("Accept", "*/*")
...
func TestWrapper(t *testing.T) {
	...
	app := web.NewHttpSever()
	app.Cfg.CopyRequestBody = true
	app.Post("/api/data", Wrapper(testFunc))

	formData := url.Values{}
	formData.Set("foo", "bar")

	marshal, _ := xml.Marshal(myStruct1)

	req := httptest.NewRequest("POST", "/api/data", bytes.NewBuffer(marshal))
	req.Header.Set("Content-Type", contentType)
	req.Header.Set("Accept", "*/*")
