...
func TestUrlFor2(t *testing.T) {
	...
	if handler.URLFor("TestController.GetURL", ":username", "cdle") != "/v1/cdle/edit" {
		logs.Info(handler.URLFor("TestController.GetURL"))
		t.Errorf("TestController.List must equal to /v1/cdle/edit")
	}
...
func TestPostFunc(t *testing.T) {
	r, _ := http.NewRequest("POST", "/cdle", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/:name", &TestController{})
	handler.ServeHTTP(w, r)
	if w.Body.String() != "cdle" {
		t.Errorf("post func should cdle")
	}
}
